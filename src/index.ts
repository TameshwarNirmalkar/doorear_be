...
const credentials = {
    key: fs.readFileSync(
        path.resolve(__dirname, "./common/certificate/127.0.0.1+1-key.pem")
    ),
    cert: fs.readFileSync(
        path.resolve(__dirname, "./common/certificate/127.0.0.1+1.pem")
    ),
};

const server = https
    .createServer(credentials, app)
    .listen(PORT, HOST, 3001, () => {
        PinoLogger.info(
            `\n\nServer ✔ (${NODE_ENV}) running on port https://${HOST}:${PORT}/${API_PARAM}/api/v2`
        );
    });
...
process.on("unhandledRejection", async (reason, promise) => {
    ...
    PinoLogger.fatal(
        `unhandledRejection Rejection at: ${catchRejection}, reason: ${reason}`
    );
...
process.on("uncaughtException", (reason, promise) => {
    PinoLogger.fatal(
        `uncaughtException Rejection at: ${promise}, reason: ${reason.message}`
    );
    // Application specific logging, error reporting, etc.
    // Optionally exit the process after logging: process.exit(1);
});
