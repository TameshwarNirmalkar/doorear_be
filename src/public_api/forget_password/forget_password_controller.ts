...
class ForgetPassword {
    public PostForgetPassword: RequestHandler = async (
        req: Request,
        res: Response
    ): Promise<void> => {
        ...
        try {
            if (clientId > 0) {
                ...
                const [effectedClientRows] =
                    await PoolConnectionInstance.query<ResultSetHeader>(
                        sqlQuery,
                        [emailaddress]
                    );
                ...
                if (affectedRows > 0) {
                    const [clientPasswordRes] =
                        await PoolConnectionInstance.query<RowDataPacket[]>(
                            `call ${DbName}.new_uspForgotPassword(?,?)`,
                            [emailaddress, clientId]
                        );
                    const { EmailBody, EmailSubject, Success } =
                        clientPasswordRes[0][0];
